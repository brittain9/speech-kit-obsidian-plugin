...
interface TranslationStatusUpdateOptions {
...
export class TranslationStatusController {
  ...
  private readonly visibleText: HTMLElement;
  private readonly liveRegion: HTMLElement;
  private reopen: (() => void) | null = null;
  private focusOnReveal = false;
  ...
  constructor(private readonly element: HTMLElement) {
    ...
    this.visibleText = this.button.createSpan({
      attr: { 'aria-hidden': 'true' },
      cls: 'local-stt-translation-status__label',
    });
    this.liveRegion = this.element.createSpan({
      attr: {
        'aria-atomic': 'true',
        'aria-live': 'polite',
        role: 'status',
      },
      cls: 'local-stt-translation-status__live',
    });
    this.button.addEventListener('click', () => {
      this.focusOnReveal = this.element.ownerDocument.activeElement === this.button;
      this.reopen?.();
    });
    this.clear();
  }
  ...
  update(
    ...
    if (state === null) {
      this.clear(options.preserveFocus ?? false);
      return;
    }
  ...
  private clear(preserveFocus = false): void {
    if (!preserveFocus) this.focusOnReveal = false;
    this.reopen = null;
    this.button.disabled = true;
    this.button.removeAttribute('aria-label');
    this.button.removeAttribute('title');
    this.visibleText.textContent = '';
    this.liveRegion.textContent = '';
    this.element.toggle(false);
  }

  private restoreFocus(): void {
    if (!this.focusOnReveal) return;
    this.focusOnReveal = false;
    this.button.focus();
  }
}
