...
describe('resolvePluginSettings', () => {
  ...
  it('defaults missing schemaVersion to the current settings schema', () => {
    expect(resolvePluginSettings({}).schemaVersion).toBe(9);
  });

  it('defaults and normalizes the HY-MT2 style instruction', () => {
  ...
  it('defaults finalized-utterance auto-copy off and honors only a persisted boolean opt-in', () => {
    ...
    expect(resolvePluginSettings({ autoCopyFinalizedUtterances: true }).schemaVersion).toBe(9);
  });
