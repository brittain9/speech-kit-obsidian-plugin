...
describe('LLM Secret Storage integration', () => {
  ...
  it('requests one normalized rewrite for settings schemas before version 9', () => {
    const result = loadPluginSettings(
      { schemaVersion: 6 },
      { getSecret: () => null, setSecret: vi.fn() },
    );

    expect(result.settings.schemaVersion).toBe(9);
    expect(result.shouldPersist).toBe(true);
  });

  it('does not rewrite already-normalized schema 9 settings', () => {
